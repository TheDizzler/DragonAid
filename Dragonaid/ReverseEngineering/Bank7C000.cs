...
namespace AtomosZ.DragonAid.ReverseEngineering
{
	...
	internal class Bank7C000
	{
		...
		internal class NMI
		{
			...
			private static void APU_UpdateSequence()
			{
				...
				if (hasCarry)
				{
					...
					{
						int address = zeroPage[ZeroPage.APU_TrackPosition + x + 0]
							+ zeroPage[ZeroPage.APU_TrackPosition + x + 1] << 8;
						a = romData[address];
						a ^= 0xFC;
						if (a == 0)
							break;
					...
					{
						a = zeroPage[ZeroPage.APU_TrackInstructions + x + 2];
						if (a != 0)
							break;
						x -= 2;
					}
				...
				x = 0x02;
				y = 0x04;
				if (zeroPage[ZeroPage.APU_TrackInstructions + 10] != 0
					&& (zeroPage[0xFA] & 0x20) != 0)
				{
				...
				y = 0x08;
				APU_CheckIfNextNote(x, y);
				a = zeroPage[0xFB]; // APU_Tri_Linear_Store
				a ^= 0xFF;
				a &= 0xFE;
				...
				if (zeroPage[ZeroPage.APU_TrackInstructions + 10] != 0)
				{
					a = zeroPage[0xFA]; // APU_NewSeq_20or00
					a &= 0x20;
					if (a == 0)
					{
						APU_Track_Vector_ClearBit7();
			...
			private static void APU_DutySetup(byte x)
			{
				...
				byte a = zeroPage[ZeroPage.APU_TempDutySettings + x + 1]; // APU_DutySetup_Vector
				if (a > 0x80)
				{
				...
				{ // track 0, 2, 4, 6
					zeroPage[ZeroPage.APU_TrackInstructions + 1] = (byte)(a & 0x0F);
					zeroPage[ZeroPage.APU_TrackInstructions + 0] = nesRam[0x047F];
					zeroPage[ZeroPage.APU_TrackInstructions + 0] = APU_GetFinalDuty();
					a &= 0xF0;
					a &= zeroPage[ZeroPage.APU_TrackInstructions + 0];
				}
				//APU_SetDuty()
			...
			{
				byte y = zeroPage[ZeroPage.APU_TempDutySettings + x + 0];
				zeroPage[ZeroPage.APU_TrackInstructions + 0]
					= romData[Pointers.ROM.APU_DutySetup_Addresses.iNESAddress + y + 0];
				zeroPage[ZeroPage.APU_TrackInstructions + 1]
					= romData[Pointers.ROM.APU_DutySetup_Addresses.iNESAddress + y + 1];

				int dutySettingsAddress = zeroPage[ZeroPage.APU_TrackInstructions + 0]
					+ zeroPage[ZeroPage.APU_TrackInstructions + 1 << 8];
				byte a;
				while (true)
				{
					...
					if (a >= 0x30)
						break;
					zeroPage[ZeroPage.APU_TempDutySettings + x + 1] = a;
					y = a;
				}

				++zeroPage[ZeroPage.APU_TempDutySettings + x + 1];
				x = a;
			}

			private static void APU_Sub_Update_A(byte a)
			{
				ASMHelper.BIT(a, zeroPage[0xFA], out bool n, out bool v, out bool z);
				if (v)
					return;
				a = zeroPage[0xFA];
				a = (byte)(a - 0x01);
			...
			{
				return (zeroPage[ZeroPage.APU_TrackInstructions + x + 2] != 0);
			}
			...
			{
				if (zeroPage[ZeroPage.APU_TrackInstructions + x + 12] != 0
					&& --zeroPage[ZeroPage.APU_TrackInstructions + x + 12] == 0)
				{
					byte a = zeroPage[ZeroPage.APU_TempDutySettings + x + 1]; // APU_DutySetup_Vector
					if (a != 0xFF)
					{
						zeroPage[ZeroPage.APU_TempDutySettings + x + 1] = 0xFE; // APU_DutySetup_Vector
						if (APU_SetOrClearCarry(x))
							nesRam[0x06F4] = 0xFF;
					}
				}
				// APU_UpdateTrack_cont_
				if (--zeroPage[ZeroPage.APU_TrackInstructions + x + 2] != 0)// when this reaches 0, update track. Note length tracker?
					return;
				//B503
				if (x == 0x00)
				...
				else if (x == 0x02 && zeroPage[ZeroPage.APU_TrackInstructions + 10] == 0)
					APU_DisableSweep(Registers.SQ1Sweep); // this probably never happens?
				APU_ParseNextTrackByte(zeroPage[ZeroPage.APU_TrackInstructions + 10], x, y);
			}
			...
			private static void APU_ParseNextTrackByte(byte a, byte x, byte y)
			{
				...
				switch (nextTrackByte)
				{
					...
					case 0xFD: // move track position to stored address at 0x06F7
						zeroPage[ZeroPage.APU_TrackPosition + x + 0] = nesRam[0x06F7 + x + 0];
						zeroPage[ZeroPage.APU_TrackPosition + x + 1] = nesRam[0x06F7 + x + 1];
					...
					case 0xFA: /// Rewind track by ($FF - $YY bytes) [$FA $XX $YY]. $XX is stored sometimes.
						...
						{
							hasCarry = false;
							zeroPage[ZeroPage.APU_TrackPosition + x + 0]
								= ASMHelper.ADC(nextTrackByte, zeroPage[ZeroPage.APU_TrackPosition + x + 0], ref hasCarry);
							if (!hasCarry)
								--zeroPage[ZeroPage.APU_TrackPosition + x + 1];
						}
					...
					case 0xEF:
						zeroPage[0xFA] = (byte)((zeroPage[0xFA] & 0x20) | 0x44); // == 44 or 64
						APU_ParseNextTrackByte(a, x, y);
						break;
					...
					default: // below 0xE1
						...
						{
							a = 0xFF;
							zeroPage[ZeroPage.APU_TempDutySettings + x + 1] = a; // APU_DutySetup_Vector
						}
						else if (a < 0x49)
						{
							if (y != 0)
							...
							if (APU_SetOrClearCarry(x))
								nesRam[0x06F4] = 0x00;
							a = zeroPage[ZeroPage.APU_TrackInstructions + x + 13];
							if (a < 0x80)
							{
								a = 0x0;
								zeroPage[ZeroPage.APU_TempDutySettings + x + 1] = a; // APU_DutySetup_Vector
						...
						if (x == 0x02 || x == 0x04) // probably 2 is SQ1 and 4 is triangle?
						{
							...
							if (a != 0)
							{
								zeroPage[ZeroPage.APU_TrackInstructions + x + 3] = zeroPage[ZeroPage.APU_TrackInstructions + x + 1];
								zeroPage[ZeroPage.APU_TrackInstructions + x + 13] = zeroPage[ZeroPage.APU_TrackInstructions + x + 11];
							}
						}
						// APU_All_Tracks
						...
						{
							nextTrackByte = APU_ReadNextTrackByte(x);
							zeroPage[ZeroPage.APU_TrackInstructions + x + 3] = (byte)(nextTrackByte & 0x7F);
							if (nextTrackByte >= 0x80)
							{
								nextTrackByte = APU_ReadNextTrackByte(x);
								zeroPage[ZeroPage.APU_TrackInstructions + x + 13] = nextTrackByte;
							}
						}
						// _APU_ParseTrackByte_End_

						zeroPage[ZeroPage.APU_TrackInstructions + x + 2] // this effects whether the track gets updated next frame?
							= zeroPage[ZeroPage.APU_TrackInstructions + x + 3];
						a = zeroPage[ZeroPage.APU_TrackInstructions + x + 13];
						a &= 0x7F;
						a = APU_Track04_SpecialOps(a, x);
						zeroPage[ZeroPage.APU_TrackInstructions + x + 12] = a;
						break;
				}
			}
			...
			private static byte APU_Track04_SpecialOps(byte a, byte x)
			{
				if (x != 0x04)
					return a;
				zeroPage[ZeroPage.APU_TrackInstructions + 1] = a;
				a = nesRam[0x047F];
				a &= 0x1F;
				if (a >= 0x06)
				{ // _L7B6F7_
					zeroPage[ZeroPage.APU_TrackInstructions + 0] = a;
					a = APU_GetFinalDuty();
					if (a != 0)
						return a;
				}
					...
					a = 0x00; // what's the point of this?? I wonder if this is a bug?
							  // _L7B700_
				zeroPage[ZeroPage.APU_TempDutySettings + 1 + x] = 0xFF;
				a = 0x01;
				return a;
			}
			...
			{
				byte a = zeroPage[ZeroPage.APU_TrackInstructions + 0];
				a ^= 0xFF; // invert byte
				a <<= 3;
				zeroPage[ZeroPage.APU_TrackInstructions + 0] = a;
				a = 0;
				zeroPage[ZeroPage.APU_TrackInstructions + 0]
					= ASMHelper.ASL(zeroPage[ZeroPage.APU_TrackInstructions + 0], 1, out bool hasCarry);
				if (!hasCarry)
					a = ASMHelper.ADC(a, zeroPage[ZeroPage.APU_TrackInstructions + 1], ref hasCarry);

				for (int i = 0; i < 4; ++i)
				{
					zeroPage[ZeroPage.APU_TrackInstructions + 1] >>= 1;
					zeroPage[ZeroPage.APU_TrackInstructions + 0]
						= ASMHelper.ASL(zeroPage[ZeroPage.APU_TrackInstructions + 0], 1, out hasCarry);
					if (!hasCarry)
						a = ASMHelper.ADC(a, zeroPage[ZeroPage.APU_TrackInstructions + 1], ref hasCarry);
				}
			...
			private static byte APU_ReadNextTrackByte(byte x)
			{
				int address = zeroPage[ZeroPage.APU_TrackPosition + x + 0]
							+ zeroPage[ZeroPage.APU_TrackPosition + x + 1] << 8;
				if (++zeroPage[ZeroPage.APU_TrackPosition + x] == 0)
					++zeroPage[ZeroPage.APU_TrackPosition + x + 1];
		...
		{
			byte a = zeroPage[ZeroPage.dynamicSubroutine_21 + 1];
			a <<= 1;
			byte x = a;
			int addr = romData[ROM.LocalPointers_78000.iNESAddress + x] + (romData[ROM.LocalPointers_78000.iNESAddress + x + 1] << 8);
