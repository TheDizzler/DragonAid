...
namespace AtomosZ.DragonAid.SpriteAid
{
	public partial class SpriteAidForm : Form
	{
		...
		private void DisplayDayNightPalette(byte[] romData)
		{
			...
			byte paletteOffset = romData[PointerList.TimeOfDayDayNightPalettesIndices.offset + timeIndex];

			for (byte i = 0; i < 0x0C; ++i)
			{
				byte color = romData[PointerList.DayNightPalettes.offset + paletteOffset + i];
				byte paletteIndex = romData[PointerList.PaletterStoreOffsets.offset + i];
				if (paletteIndex >= 4)
					++paletteIndex;
				if (paletteIndex >= 8)
					++paletteIndex;
		...
		private void SetupDayCycleTimers(byte[] romData)
		{
			...
			{
				timeSpinners[timeIndex].Value = romData[PointerList.TimeOfDayChangeTimes.offset + timeIndex];
			}
		}
